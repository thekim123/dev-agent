...
- threshold를 임의로 정하는 게 아니라 **실제 점수 분포를 실험한 후 근거를 가지고 정해야 한다**는 것을 체감했다.
- `space_type` 설정이 점수 분포에 직접 영향을 준다는 것을 l2 → cosinesimil 전환으로 확인했다.
- embedding 모델 특성상 완전 무관한 질문도 높은 점수가 나올 수 있다는 한계를 이해했다.
- threshold 하나로 완벽하게 필터링하는 건 비현실적이고, **top-k + 낮은 threshold + LLM 후처리** 조합이 현실적 전략이라는 점을 인식했다.
- 인덱스 설정 변경 시 재생성이 필요하다는 것을 "데이터 마이그레이션" 관점에서 인식했다.
